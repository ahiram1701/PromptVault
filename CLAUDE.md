...
That check **retries each file** (4 attempts, 4 s apart, fresh cache-buster each time) instead of failing on the first mismatch. The CLI uploads to a new folder and repoints the subdomain *afterwards*, so for a few seconds the URL still serves the previous deployment — unevenly: on a real deploy `debug.html` was already the new one while `app.js`, `storage.js` and `styles.css` were still the old ones, at their exact previous byte sizes. Verifying immediately reported three DIFFs that did not exist and looked like a broken deploy. Don't remove the retry to make the script finish faster; a genuinely broken deploy still fails, just ~2 min later, and a good one costs nothing extra.
